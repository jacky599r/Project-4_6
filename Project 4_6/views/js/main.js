...
var adjectives = ["dark", "color", "whimsical", "shiny", "noise", "apocalyptic", "insulting", "praise", "scientific"],
    ...
    resizePizzas = function(size) {
        ...
        // Returning an array-like collection of pizza elenents, and iterate them on the page as well as change their widths
        function changePizzaSizes(size) {
          var selectedContainers = document.getElementsByClassName("randomPizzaContainer");
          var dx = determineDx(selectedContainers[0], size);
          var newwidth = (selectedContainers[0].offsetWidth + dx) + 'px';

          for (var i = 0; i < selectedContainers.length; i++) {
            selectedContainers[i].style.width = newwidth;
          }
        }
